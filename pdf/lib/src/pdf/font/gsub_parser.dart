...
class ScriptRecord {
  ...
  static ScriptRecord parse(ByteData data, int recordOffset, int baseOffset) {
    ...
    var langSysRecordOffset = tableOffset + 4;
    final langSysRecords = <LangSysRecord>[];
...
class LangSysTable {
  ...
  static LangSysTable parse(ByteData data, int offset) {
    ...
    final featureIndexes = <int>[];
    var featureIndexOffset = offset + 6;
...
class ScriptList {
  ...
  static ScriptList parse(ByteData data, GsubHeader header) {
    ...
    var scriptRecordOffset = header.scriptListOffset + 2;
    final scriptRecords = <ScriptRecord>[];
...
class FeatureList {
  ...
  static FeatureList parse(ByteData data, GsubHeader header) {
    ...
    var featureRecordOffset = header.featureListOffset + 2;
    final featureRecords = <FeatureRecord>[];
...
class Lookup {
  ...
  static Lookup parse(ByteData data, int offset) {
    ...
    pointer += 2;
    final subTables = <dynamic>[];
    if (subTableCount > 0) {
      final subTableBase = offset + pointer;
      pointer += 2;
      for (var i = 0; i < subTableCount; i++) {
        final subTableOffsets = offset + data.getUint16(subTableBase + 2 * i);
        final table = SubTable.parse(data, subTableOffsets, lookupType);
...
class LigatureSubstitution {
  ...
  static LigatureSubstitution parse(ByteData data, int offset) {
    ...
    final ligatureSet = <LigatureSet>[];
    pointer += 2;
    if (ligatureSetCount > 0) {
      final ligatureSetBase = offset + pointer;
      pointer += 2;
      for (var i = 0; i < ligatureSetCount; i++) {
        final ligatureSetOffset =
            offset + data.getUint16(ligatureSetBase + 2 * i);
        final ligSet = LigatureSet.parse(data, ligatureSetOffset);
...
class LigatureSet {
  ...
  static LigatureSet parse(ByteData data, int offset) {
    ...
    if (ligatureCount > 0) {
      final ligatureBase = offset + pointer;
      pointer += 2;
      for (var i = 0; i < ligatureCount; i++) {
        final ligatureOffset = offset + data.getUint16(ligatureBase + 2 * i);
        final ligature = Ligature.parse(data, ligatureOffset);
...
class Coverage {
  ...
  static Coverage parse(ByteData data, int offset) {
    ...
    if (format == 1) {
      ...
      pointer += 2;
      if (glyphCount > 0) {
        var glyphsOffset = offset + pointer;
        for (var i = 0; i < glyphCount; i++) {
      ...
      pointer += 2;
      if (rangeCount > 0) {
        var rangeRecordOffset = offset + pointer;
        for (var i = 0; i < rangeCount; i++) {
          final record = RangeRecord.parse(data, rangeRecordOffset);
    ...
    if (substFormat == 1) {
      ...
      if (chainCount > 0) {
        ...
        for (var i = 0; i < chainCount; i++) {
          ...
          final chainRule = ChainRuleSets.parse(data, chainRuleOffset);
      ...
      final backtrackClassDefOffset = offset + data.getUint16(offset + pointer);
      pointer += 2;
      backtrackClassDef = ClassDef.parse(data, backtrackClassDefOffset);

      final inputClassDefOffset = offset + data.getUint16(offset + pointer);
      pointer += 2;
      inputClassDef = ClassDef.parse(data, inputClassDefOffset);

      final lookaheadClassDefOffset = offset + data.getUint16(offset + pointer);
      pointer += 2;
      ...
      pointer += 2;
      if (chainCount > 0) {
        final chainClassBase = offset + pointer;
        chainClassSet = [];
        for (var i = 0; i < chainCount; i++) {
          final chainClassOffset =
      ...
      if (backtrackGlyphCount > 0) {
        final backtrackBase = offset + pointer;
        pointer += backtrackGlyphCount * 2;
        backtrackCoverage = [];
        for (var i = 0; i < backtrackGlyphCount; i++) {
          final backtrackOffset =
              offset + data.getUint16(backtrackBase + 2 * i);
          final coverage = Coverage.parse(data, backtrackOffset);
      ...
      if (inputGlyphCount > 0) {
        final inputBase = offset + pointer;
        pointer += inputGlyphCount * 2;
        inputCoverage = [];
        for (var i = 0; i < inputGlyphCount; i++) {
          final inputOffset = offset + data.getUint16(inputBase + 2 * i);
          final coverage = Coverage.parse(data, inputOffset);
      ...
      if (lookaheadGlyphCount > 0) {
        final lookaheadBase = offset + pointer;
        pointer += lookaheadGlyphCount * 2;
        lookaheadCoverage = [];
        for (var i = 0; i < lookaheadGlyphCount; i++) {
          final lookaheadOffset =
              offset + data.getUint16(lookaheadBase + 2 * i);
          final coverage = Coverage.parse(data, lookaheadOffset);
      ...
      pointer += 2;
      var lookupOffset = offset + pointer;
      lookupRecords = [];
...
class ChainRuleSets {
  ...
  static ChainRuleSets parse(ByteData data, int offset) {
    var pointer = 0;
    final chainRuleCount = data.getUint16(offset);
    pointer += 2;
...
class ChainRule {
  ...
  static ChainRule parse(ByteData data, int offset) {
    var pointer = 0;
    final backtrackGlyphCount = data.getUint16(offset);
    pointer += 2;
    final backtrack = <int>[];
    if (backtrackGlyphCount > 0) {
      var backtrackOffset = offset + pointer;
      for (var i = 0; i < backtrackGlyphCount; i++) {
    ...
    final inputGlyphCount = data.getUint16(offset + pointer) - 1;
    pointer += 2;
    final input = <int>[];
    if (inputGlyphCount > 0) {
      var inputOffset = offset + pointer;
      for (var i = 0; i < inputGlyphCount; i++) {
    ...
    final lookaheadGlyphCount = data.getUint16(offset + pointer);
    pointer += 2;
    final lookahead = <int>[];
    if (lookaheadGlyphCount > 0) {
      var lookaheadOffset = offset + pointer;
      for (var i = 0; i < lookaheadGlyphCount; i++) {
    ...
    final lookupCount = data.getUint16(offset + pointer);
    pointer += 2;
    final lookupRecords = <LookupRecord>[];
    if (lookupCount > 0) {
      var lookupOffset = offset + pointer;
      for (var i = 0; i < lookupCount; i++) {
        final record = LookupRecord.parse(data, lookupOffset);
...
class LookupRecord {
  ...
  static LookupRecord parse(ByteData data, int offset) {
    var pointer = 0;
    final sequenceIndex = data.getUint16(offset);
    pointer += 2;
    final lookupListIndex = data.getUint16(offset + pointer);
    pointer += 2;
...
class ClassDef {
  ...
  static ClassDef parse(ByteData data, int offset) {
    var pointer = 0;
    final classDefFormat = data.getUint16(offset);
    pointer += 2;
    ...
    if (classDefFormat == 1) {
      ...
      if (glyphCount > 0) {
        classValueArray = [];
        var classValueOffset = offset + pointer;
        for (var i = 0; i < glyphCount; i++) {
      ...
      if (classRangeCount > 0) {
        var classRecordOffset = offset + pointer;
        classRangeRecords = [];
        for (var i = 0; i < classRangeCount; i++) {
          final record = ClassRangeRecord.parse(data, classRecordOffset);
...
class ClassRangeRecord {
  ...
  static ClassRangeRecord parse(ByteData data, int offset) {
    ...
    final end = data.getUint16(offset + pointer);
    pointer += 2;
    final classValue = data.getUint16(offset + pointer);
    pointer += 2;
...
class ReverseChainedContextualSingleSubstitutionSubTable {
  ...
  static ReverseChainedContextualSingleSubstitutionSubTable parse(
    ...
    final backtrackCoverageOffsets = <int>[];
    for (var i = 0; i < backtrackGlyphCount; i++) {
    ...
    final lookaheadCoverageOffsets = <int>[];
    for (var i = 0; i < lookaheadGlyphCount; i++) {
    ...
    final substituteGlyphIDs = <int>[];
    for (var i = 0; i < substituteGlyphCount; i++) {
...
class LookupList {
  ...
  static LookupList parse(ByteData data, int offset) {
    ...
    var lookupOffset = offset + 2;
    final lookups = <Lookup>[];

    for (var i = 0; i < lookupCount; i++) {
      final lookupOffsetOffset = data.getUint16(lookupOffset);
